  ...
  function render(){
    ...
    foreach($weeks as $week){
      ...
      foreach($days as $day){
        ...
        $weekDay = Carbon::parse($day->everyDay())->dayOfWeek;
        $dateTextColor = '';
        if ($weekDay === 0) {
          $dateTextColor = 'text-danger';
        } elseif ($weekDay === 6) {
          $dateTextColor = 'text-primary';
        }
        ...
        $html[] = '<p class="'.$dateTextColor.'">'.Carbon::parse($day->everyDay())->format('j日').'</p>';
        ...
        if ($day->everyDay() <= $today) {
          if (in_array($day->everyDay(), $day->authReserveDay())) {
            ...
            $html[] = '<p class="small" style="color: black;">'. str_replace('リモ', '', $reserveLabel) .'参加</p>';
        ...
        $html[] = '<span class="'.$dateTextColor.'">'.$day->getDate().'</span>';
        $html[] = '</td>';
      }
      $html[] = '</tr>';
    }
